    ...
    while (!screenDef.isNull()) {
        ...
        while (!qmlLibrary.isNull()) {
            ...
            if (path.isDir()){
                ...
                qWarning() << "Unable to add controller QML library path." << path.absolutePath() << "is not a directory or is missing";
            }
            qmlLibrary = qmlLibrary.nextSiblingElement("library");
        }

        qDebug() << "Adding screen render using file" << file << "and libpath" << libPaths;
        mapping->addQMLFile(identifier, QSize(width, height), file, libPaths, screenCount, targetFps);
