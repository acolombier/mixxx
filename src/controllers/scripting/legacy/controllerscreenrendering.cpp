...
ControllerScreenRendering::ControllerScreenRendering(Controller* controller, LegacyControllerMapping::QMLFileInfo qml, const RuntimeLoggingCategory& logger, uint8_t screenId)
        : ControllerScriptEngineBase(controller, logger), m_screenId(screenId), m_renderingInfo(qml), m_size(320, 240) {
...
void ControllerScreenRendering::start() {
    ...
    m_tranformFunction = m_qmlEngine->evaluate(QStringLiteral(
            ...
            " return data; "
            "})"));
    RELEASE_ASSERT(m_tranformFunction.isCallable());
    // m_tranformFunction = m_qmlEngine->evaluate(QStringLiteral(
    //         // arg2 is the timestamp for ControllerScriptModuleEngine.
    //         // In ControllerScriptEngineLegacy it is the length of the array.
    //         "(function(input) {"
    ...
    //         "    for (var i = 0; i < 320 * 240; i ++){"
    //         "        out.setUint16(16 + 2 * i, (in.getUint8(i * 4) & 0xf8) << 8 | (in.getUint8(i * 4 + 1) & 0xfc) << 3 | (in.getUint8(i * 4 + 2) >> 3));"
    //         "    }"
    //         "})"));
...
void ControllerScreenRendering::createFbo() {
    m_fbo = std::make_unique<QOpenGLFramebufferObject>(m_size * 1.0, QOpenGLFramebufferObject::CombinedDepthStencil);
...
bool ControllerScreenRendering::load(const QString& qmlFile, const QSize& size) {
    ...
    QObject* pRootObject = m_qmlComponent->createWithInitialProperties(QVariantMap{{"screenId", m_screenId}});
    if (m_qmlComponent->isError()) {
        const QList<QQmlError> errorList = m_qmlComponent->errors();
        for (const QQmlError& error : errorList)
            qWarning() << error.url() << error.line() << error;
...
void ControllerScreenRendering::renderNext() {
    ...
    if (usecToWaitBeforeFrame > 0){
        qDebug() << "Sleeping for " << durationToWaitBeforeFrame.formatMillisWithUnit() << " before render";
    ...
    QJSValue dataToSend = m_tranformFunction.call(QJSValueList{m_qmlEngine->toScriptValue(input), m_screenId});

    if (dataToSend.isError()){
        qWarning() << "Could not tranform rendering buffer. Error:" << dataToSend.toString();
        cleanup();
        return;
    }
    ...
    qDebug() << "Fame took " << (endOfRender - m_nextFrameStart).formatMillisWithUnit() << "(Render: "
        << (startOfTransform - m_nextFrameStart).formatMillisWithUnit() << "/ Tranform: " << (endOfTransform - startOfTransform).formatMillisWithUnit()
        << ") and buffer has" << output.size() << "bytes. Had error?" << dataToSend.isError();
