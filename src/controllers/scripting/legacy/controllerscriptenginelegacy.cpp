...
bool ControllerScriptEngineLegacy::initialize() {
    ...
    //     //     m_renderBindingList.append(connect(pWin, &QQuickWindow::afterRendering, this, [=](){
    //     //         QImage frame = pWin->grabWindow();
    //     //         handleRenderingData(QByteArray((const char *)frame.constBits(), frame.sizeInBytes()));
    //     //     }));
    //     // }
    // } else {
...
bool ControllerScriptEngineLegacy::handleRenderingData(const QString& identifier, uint8_t screenId, const QByteArray& data) {
    // This function is called from outside the controller engine, so we can't
    // use VERIFY_OR_DEBUG_ASSERT here
