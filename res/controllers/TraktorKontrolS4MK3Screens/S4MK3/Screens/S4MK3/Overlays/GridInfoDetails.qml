...
Item {
    ...
    property string finalValue: zoom ? (((parameter.value+1)/2)*100).toFixed(2)+"%" : toInt_round(parameter.value*100).toString() + "%"
    ...
    Widgets.ProgressBar {
        ...
        value: label == "ZOOM" ? ((parameter.value+1)/2) : parameter.value
