...
Item {
    ...
    property string newIndex:			model.keyIndex
  ...
  // visible: !ListView.isCurrentItem
    readonly property string bpm: model.bpm.toFixed(2).toString()
  ...
  // container for zebra & track infos
	...
	// track name
        Text {
            ...
            elide: Text.ElideRight
            text: settings.swapArtistTitleColumns ? ((model.dataType == BrowserDataType.Track) ? model.artistName : "") : (settings.showArtistColumn && settings.showTrackTitleColumn ? ((model.dataType == BrowserDataType.Track) ? model.trackName : "") : (!isShift && !settings.showArtistColumn && settings.showTrackTitleColumn ? ((model.dataType == BrowserDataType.Track) ? model.trackName : "") : ((model.dataType == BrowserDataType.Track) ? model.artistName : "")))
            font.pixelSize: settings.browserFontSize
            color: isLoaded ? "lime" : ((model.prevPlayed && !model.prelisten) ? "yellow" : ((model.bpm.toFixed(4) == "0.0000" ) ? "red" : textColor))
        }
	...
	// artist name
        Text {
            ...
            color: isLoaded ? "lime" : ((model.prevPlayed && !model.prelisten) ? "yellow" : ((model.bpm.toFixed(4) == "0.0000" ) ? "red" : textColor))
            clip: true
            text: settings.swapArtistTitleColumns ? (model.dataType == BrowserDataType.Track) ? model.trackName : "" : (settings.showArtistColumn && settings.showTrackTitleColumn ? ((model.dataType == BrowserDataType.Track) ? model.artistName : "") : (!isShift && settings.showArtistColumn && !settings.showTrackTitleColumn ? ((model.dataType == BrowserDataType.Track) ? model.artistName : "") : (model.dataType == BrowserDataType.Track) ? model.trackName : ""))
            font.pixelSize: settings.browserFontSize
            elide: Text.ElideRight
        }
    ...
    // bpm
        Text {
            ...
            color: settings.bpmBrowserTextColor ? (bpm == "0.00") ? "red" : (bpmMatch <= settings.browserBpmGreen) && (bpmMatch >= -(settings.browserBpmGreen)) ? "lime" : (!((bpmMatch >= settings.browserBpmRed) || (bpmMatch <= -(settings.browserBpmRed)) && (masterBPM != "0.00")) ? textColor : "orange") : textColor
            clip: true
            text: (model.dataType == BrowserDataType.Track) ? bpm : ""
            font.pixelSize: settings.browserFontSize
        }
    ...
    // key
        Text {
            ...
            color: (model.dataType == BrowserDataType.Track) ? (((model.key == "none") || (model.key == "None")) ? "White" : ((colorKey(contactDelegate.newIndex, contactDelegate.masterKeyIndex) && contactDelegate.deckPlaying) ? parent.colorForKey(model.keyIndex) : "White")) : "White"
            width: settings.hideKey ? 0 : 30
            clip: true
            text: (model.dataType == BrowserDataType.Track) ? (((model.key == "none") || (model.key == "None")) ? "n.a." : (settings.camelotKey ? utils.camelotConvert(model.key) : model.key)) : ""
            font.pixelSize: settings.browserFontSize
        }
